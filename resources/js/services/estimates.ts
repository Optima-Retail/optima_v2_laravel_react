...
export const estimatesService = {
    ...
    destroyAttachment(estimateId: number, attachmentId: number, options: Record<string, unknown> = {}) {
        router.delete(`${base}/${estimateId}/attachments/${attachmentId}`, {
            preserveScroll: true,
            ...options,
        });
    },
    ...
    async bulkStatus(payload: {
        ...
        const response = await fetch(estimatesService.bulkStatusPath, {
            method: 'POST',
