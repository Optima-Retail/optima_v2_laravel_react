...
import type { GlobalServiceTypeListItem } from '@/support/types/domain/global-service-type';

type GlobalServiceTypesIndexProps = {
    filters: {
        search: string;
        sort: string;
        direction: string;
...
export default function GlobalServiceTypesIndex({ filters, can }: GlobalServiceTypesIndexProps) {
    ...
    function buildColumns({ titleFormatter, getTable }: RemoteDataColumnHelpers): ColumnDefinition[] {
        return [
            ...
            },
            {
                ...
                headerSort: false,
                formatter: (cell: CellComponent) => {
                    const type = cell.getRow().getData() as GlobalServiceTypeListItem;
                    const parts: string[] = [];

                    if (canRef.current.update) {
                        parts.push(
                            tabulatorEditLink(
                                globalServiceTypesService.editPath(type.id),
                ...
                cellClick: async (event: UIEvent, cell: CellComponent) => {
                    ...
                    globalServiceTypesService.destroy(type.id, {
                        preserveScroll: true,
                        onSuccess: () => {
    ...
    return (
        <AppLayout title={t('nav.types')}>
            ...
            <div className="space-y-6">
                <PageHeader
                    title={t('nav.types')}
                    description={t('globalServiceTypes.description')}
                    actions={
                        can.create ? (
                            <Link
                                href={globalServiceTypesService.createPath}
                            ...
                            >
                                ...
                                {t('globalServiceTypes.new')}
                            </Link>
                        ) : null
                    }
                />

                <TypesConfigTabs activeId="global-service" />

                <RemoteDataTable<GlobalServiceTypeListItem>
                    ref={tableRef}
                    ajaxURL={globalServiceTypesService.dataPath}
                    ...
                    filterFields={[
                        {
                            ...
                            placeholder: t('globalServiceTypes.searchPlaceholder'),
                        },
                    ]}
                    syncUrlBase={globalServiceTypesService.indexPath}
                    emptyIcon={<Globe2 className="size-5" aria-hidden />}
                    emptyMessage={t('common.empty', { resource: t('globalServiceTypes.resourcePlural') })}
                    deps={[i18n.language]}
                />
            </div>
        </AppLayout>
