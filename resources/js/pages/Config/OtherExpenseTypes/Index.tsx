...
import type { OtherExpenseTypeListItem } from '@/support/types/domain/other-expense-type';

type OtherExpenseTypesIndexProps = {
    filters: {
        search: string;
        sort: string;
        direction: string;
...
export default function OtherExpenseTypesIndex({ filters, can }: OtherExpenseTypesIndexProps) {
        ...
        return [
            ...
            },
            {
                ...
                formatter: (cell: CellComponent) => {
                    const type = cell.getRow().getData() as OtherExpenseTypeListItem;
                    const parts: string[] = [];

                    if (canRef.current.update) {
                        parts.push(
                            tabulatorEditLink(
                                otherExpenseTypesService.editPath(type.id),
                ...
                cellClick: async (event: UIEvent, cell: CellComponent) => {
                    ...
                    otherExpenseTypesService.destroy(type.id, {
                        preserveScroll: true,
                        onSuccess: () => {
    ...
    return (
        <AppLayout title={t('nav.types')}>
            ...
            <div className="space-y-6">
                <PageHeader
                    title={t('nav.types')}
                    description={t('otherExpenseTypes.description')}
                    actions={
                        can.create ? (
                            <Link
                                href={otherExpenseTypesService.createPath}
                            ...
                            >
                                ...
                                {t('common.newItem', { resource: t('otherExpenseTypes.resource') })}
                            </Link>
                        ) : null
                    }
                />

                <TypesConfigTabs activeId="other-expense" />

                <RemoteDataTable<OtherExpenseTypeListItem>
                    ref={tableRef}
                    ajaxURL={otherExpenseTypesService.dataPath}
                    ...
                    filterFields={[
                        {
                            ...
                            placeholder: t('otherExpenseTypes.searchPlaceholder'),
                        },
                    ]}
                    syncUrlBase={otherExpenseTypesService.indexPath}
                    emptyIcon={<Receipt className="size-5" aria-hidden />}
                    emptyMessage={t('common.empty', { resource: t('otherExpenseTypes.resourcePlural') })}
                    deps={[i18n.language]}
                />
            </div>
        </AppLayout>
