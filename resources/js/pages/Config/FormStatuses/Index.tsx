...
import type { FormStatusListItem } from '@/support/types/domain/form-status';

type FormStatusesIndexProps = {
    filters: {
        search: string;
        sort: string;
        direction: string;
...
export default function FormStatusesIndex({ filters, can }: FormStatusesIndexProps) {
    ...
    function buildColumns({ titleFormatter, getTable }: RemoteDataColumnHelpers): ColumnDefinition[] {
        return [
            ...
            {
                title: t('formStatuses.nextStatus'),
                field: 'next_status_name',
                minWidth: 180,
                headerSort: false,
                titleFormatter,
                formatter: (cell: CellComponent) => {
                    const row = cell.getRow().getData() as FormStatusListItem;

                    return row.next_status_name ?? '—';
                },
            },
            {
            ...
            },
            {
                ...
                headerSort: false,
                formatter: (cell: CellComponent) => {
                    const status = cell.getRow().getData() as FormStatusListItem;
                    const parts: string[] = [];

                    if (canRef.current.update) {
                        parts.push(
                            tabulatorEditLink(
                                formStatusesService.editPath(status.id),
                ...
                cellClick: async (event: UIEvent, cell: CellComponent) => {
                    ...
                    formStatusesService.destroy(status.id, {
                        preserveScroll: true,
                        onSuccess: () => {
        ...
        <AppLayout title={t('nav.statuses')}>
            ...
            <div className="space-y-6">
                <PageHeader
                    ...
                    description={t('formStatuses.description')}
                    actions={
                        can.create ? (
                            <Link
                                href={formStatusesService.createPath}
                ...
                <StatusConfigTabs activeId="form" />

                <RemoteDataTable<FormStatusListItem>
                    ref={tableRef}
                    ajaxURL={formStatusesService.dataPath}
                    ...
                    filterFields={[
                        {
                            ...
                            placeholder: t('formStatuses.searchPlaceholder'),
                        },
                    ]}
                    syncUrlBase={formStatusesService.indexPath}
                    emptyIcon={<CircleDot className="size-5" aria-hidden />}
                    emptyMessage={t('common.empty', { resource: t('formStatuses.resourcePlural') })}
                    deps={[i18n.language]}
                />
            </div>
        </AppLayout>
