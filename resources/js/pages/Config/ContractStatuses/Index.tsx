...
import type { ContractStatusListItem } from '@/support/types/domain/contract-status';

type ContractStatusesIndexProps = {
    filters: {
        search: string;
        sort: string;
        direction: string;
...
export default function ContractStatusesIndex({ filters, can }: ContractStatusesIndexProps) {
    ...
    function buildColumns({ titleFormatter, getTable }: RemoteDataColumnHelpers): ColumnDefinition[] {
        return [
            ...
            {
                title: t('contractStatuses.lifecycle'),
                field: 'lifecycle',
                width: 120,
                headerSort: true,
                titleFormatter,
            ...
            {
                title: t('contractStatuses.isOpen'),
                field: 'is_open',
                width: 120,
                headerSort: true,
                titleFormatter,
            ...
            },
            {
                ...
                headerSort: false,
                formatter: (cell: CellComponent) => {
                    const status = cell.getRow().getData() as ContractStatusListItem;
                    const parts: string[] = [];

                    if (canRef.current.update) {
                        parts.push(
                            tabulatorEditLink(
                                contractStatusesService.editPath(status.id),
                ...
                cellClick: async (event: UIEvent, cell: CellComponent) => {
                    ...
                    contractStatusesService.destroy(status.id, {
                        preserveScroll: true,
                        onSuccess: () => {
        ...
        <AppLayout title={t('nav.statuses')}>
            ...
            <div className="space-y-6">
                <PageHeader
                    ...
                    description={t('contractStatuses.description')}
                    actions={
                        can.create ? (
                            <Link
                                href={contractStatusesService.createPath}
                            ...
                            >
                                ...
                                {t('contractStatuses.new')}
                            </Link>
                        ) : null
                    }
                />

                <StatusConfigTabs activeId="contract" />

                <RemoteDataTable<ContractStatusListItem>
                    ref={tableRef}
                    ajaxURL={contractStatusesService.dataPath}
                    ...
                    filterFields={[
                        {
                            ...
                            placeholder: t('contractStatuses.searchPlaceholder'),
                        },
                    ]}
                    syncUrlBase={contractStatusesService.indexPath}
                    emptyIcon={<CircleDot className="size-5" aria-hidden />}
                    emptyMessage={t('common.empty', { resource: t('contractStatuses.resourcePlural') })}
                    deps={[i18n.language]}
                />
            </div>
        </AppLayout>
