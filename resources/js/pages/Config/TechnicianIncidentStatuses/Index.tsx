...
import type { TechnicianIncidentStatusListItem } from '@/support/types/domain/technician-incident-status';

type TechnicianIncidentStatusesIndexProps = {
    filters: {
        search: string;
        sort: string;
        direction: string;
...
export default function TechnicianIncidentStatusesIndex({
    filters,
    can,
}: TechnicianIncidentStatusesIndexProps) {
    ...
    function buildColumns({ titleFormatter, getTable }: RemoteDataColumnHelpers): ColumnDefinition[] {
        return [
            ...
            {
                title: t('technicianIncidentStatuses.lifecycle'),
                field: 'lifecycle',
                width: 120,
                headerSort: true,
                titleFormatter,
            ...
            {
                title: t('technicianIncidentStatuses.isOpen'),
                field: 'is_open',
                width: 120,
                headerSort: true,
                titleFormatter,
            ...
            },
            {
                ...
                headerSort: false,
                formatter: (cell: CellComponent) => {
                    const status = cell.getRow().getData() as TechnicianIncidentStatusListItem;
                    const parts: string[] = [];

                    if (canRef.current.update) {
                        parts.push(
                            tabulatorEditLink(
                                technicianIncidentStatusesService.editPath(status.id),
                ...
                cellClick: async (event: UIEvent, cell: CellComponent) => {
                    ...
                    const confirmed = await confirmAction({
                        title: t('common.deleteTitle', {
                            resource: t('technicianIncidentStatuses.resource'),
                        }),
                    ...
                    technicianIncidentStatusesService.destroy(status.id, {
                        preserveScroll: true,
                        onSuccess: () => {
        ...
        <AppLayout title={t('nav.statuses')}>
            ...
            <div className="space-y-6">
                <PageHeader
                    ...
                    description={t('technicianIncidentStatuses.description')}
                    actions={
                        can.create ? (
                            <Link
                                href={technicianIncidentStatusesService.createPath}
                            ...
                            >
                                ...
                                {t('technicianIncidentStatuses.new')}
                            </Link>
                        ) : null
                    }
                />

                <StatusConfigTabs activeId="technician-incident" />

                <RemoteDataTable<TechnicianIncidentStatusListItem>
                    ref={tableRef}
                    ajaxURL={technicianIncidentStatusesService.dataPath}
                    ...
                    filterFields={[
                        {
                            ...
                            placeholder: t('technicianIncidentStatuses.searchPlaceholder'),
                        },
                    ]}
                    syncUrlBase={technicianIncidentStatusesService.indexPath}
