...
import type { IncidentStatusListItem } from '@/support/types/domain/incident-status';

type IncidentStatusesIndexProps = {
    filters: {
        search: string;
        sort: string;
        direction: string;
...
export default function IncidentStatusesIndex({ filters, can }: IncidentStatusesIndexProps) {
    ...
    function buildColumns({ titleFormatter, getTable }: RemoteDataColumnHelpers): ColumnDefinition[] {
        return [
            ...
            {
                title: t('incidentStatuses.lifecycle'),
                field: 'lifecycle',
                width: 120,
                headerSort: true,
                titleFormatter,
            ...
            {
                title: t('incidentStatuses.isOpen'),
                field: 'is_open',
                width: 120,
                headerSort: true,
                titleFormatter,
            ...
            },
            {
                ...
                headerSort: false,
                formatter: (cell: CellComponent) => {
                    const status = cell.getRow().getData() as IncidentStatusListItem;
                    const parts: string[] = [];

                    if (canRef.current.update) {
                        parts.push(
                            tabulatorEditLink(
                                incidentStatusesService.editPath(status.id),
                ...
                cellClick: async (event: UIEvent, cell: CellComponent) => {
                    ...
                    incidentStatusesService.destroy(status.id, {
                        preserveScroll: true,
                        onSuccess: () => {
        ...
        <AppLayout title={t('nav.statuses')}>
            ...
            <div className="space-y-6">
                <PageHeader
                    ...
                    description={t('incidentStatuses.description')}
                    actions={
                        can.create ? (
                            <Link
                                href={incidentStatusesService.createPath}
                            ...
                            >
                                ...
                                {t('incidentStatuses.new')}
                            </Link>
                        ) : null
                    }
                />

                <StatusConfigTabs activeId="incident" />

                <RemoteDataTable<IncidentStatusListItem>
                    ref={tableRef}
                    ajaxURL={incidentStatusesService.dataPath}
                    ...
                    filterFields={[
                        {
                            ...
                            placeholder: t('incidentStatuses.searchPlaceholder'),
                        },
                    ]}
                    syncUrlBase={incidentStatusesService.indexPath}
                    emptyIcon={<CircleDot className="size-5" aria-hidden />}
                    emptyMessage={t('common.empty', { resource: t('incidentStatuses.resourcePlural') })}
                    deps={[i18n.language]}
                />
            </div>
        </AppLayout>
