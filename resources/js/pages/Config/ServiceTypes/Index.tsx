...
import type { ServiceTypeListItem } from '@/support/types/domain/service-type';

type ServiceTypesIndexProps = {
    filters: {
        search: string;
        sort: string;
        direction: string;
...
export default function ServiceTypesIndex({ filters, can }: ServiceTypesIndexProps) {
    ...
    function buildColumns({ titleFormatter, getTable }: RemoteDataColumnHelpers): ColumnDefinition[] {
        return [
            ...
            },
            {
                ...
                headerSort: false,
                formatter: (cell: CellComponent) => {
                    const type = cell.getRow().getData() as ServiceTypeListItem;
                    const parts: string[] = [];

                    if (canRef.current.update) {
                        parts.push(
                            tabulatorEditLink(
                                serviceTypesService.editPath(type.id),
                ...
                cellClick: async (event: UIEvent, cell: CellComponent) => {
                    ...
                    serviceTypesService.destroy(type.id, {
                        preserveScroll: true,
                        onSuccess: () => {
            ...
            <div className="space-y-6">
                <PageHeader
                    ...
                    actions={
                        can.create ? (
                            <Link
                                href={serviceTypesService.createPath}
                ...
                <TypesConfigTabs activeId="service" />

                <RemoteDataTable<ServiceTypeListItem>
                    ref={tableRef}
                    ajaxURL={serviceTypesService.dataPath}
                    ...
                    syncUrlBase={serviceTypesService.indexPath}
                    emptyIcon={<Wrench className="size-5" aria-hidden />}
                    emptyMessage={t('common.empty', { resource: t('serviceTypes.resourcePlural') })}
                    deps={[i18n.language]}
                />
            </div>
        </AppLayout>
