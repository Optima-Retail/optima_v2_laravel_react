...
import type { EstablishmentAttachmentItem } from '@/support/types/domain/establishment';

type EstablishmentAttachmentsPanelProps = {
...
function AttachmentSection({
    ...
    isPrivate: boolean;
    establishmentId: number;
    onPreview: (attachment: EstablishmentAttachmentItem) => void;
}) {
    ...
    function upload(file: File | null | undefined) {
        ...
        setUploading(true);
        establishmentsService.storeAttachment(establishmentId, file, isPrivate, {
    ...
    async function remove(attachment: EstablishmentAttachmentItem) {
        if (!canDelete) {
            return;
        }

        const confirmed = await confirmAction({
            title: t('common.deleteTitle', { resource: t('establishments.attachments.attachment') }),
        ...
        establishmentsService.destroyAttachment(establishmentId, attachment.id);
    }
                ...
                {canUpload ? (
                    <div>
                        ...
                        >
                            ...
                            {t('establishments.attachments.upload')}
                        </Button>
                    </div>
                ) : null}
            </div>
                        ...
                        >
                            ...
                            <div className="flex shrink-0 items-center gap-2">
                                {attachment.is_image && canDownload ? (
                                    <Button type="button" variant="secondary" onClick={() => onPreview(attachment)}>
                                ...
                                {canDelete ? (
                                    <Button type="button" variant="danger" onClick={() => remove(attachment)}>
...
export function EstablishmentAttachmentsPanel({
    establishmentId,
    attachments,
    can,
}: EstablishmentAttachmentsPanelProps) {
    const { t } = useTranslation();
    const [preview, setPreview] = useState<EstablishmentAttachmentItem | null>(null);
    ...
    return (
        <>
            <div className="space-y-5">
                <AttachmentSection
                    title={t('establishments.attachments.publicTitle')}
                    description={t('establishments.attachments.publicDescription')}
                    emptyLabel={t('establishments.attachments.publicEmpty')}
                ...
                {can.view_private_attachments ? (
                    <AttachmentSection
                        title={t('establishments.attachments.privateTitle')}
                        description={t('establishments.attachments.privateDescription')}
                        emptyLabel={t('establishments.attachments.privateEmpty')}
            ...
            <AttachmentImagePreviewModal
                open={preview !== null}
                title={preview?.name ?? t('establishments.attachments.attachment')}
