...
export function ClientArticlesModal({
    open,
    relationshipId,
    clientName,
    canEdit,
    onClose,
}: ClientArticlesModalProps) {
    ...
    const visibleRows = useMemo(() => {
        ...
        return rows.filter((row) => {
            const label = (optionLabelById.get(row.article_id) ?? '').toLocaleLowerCase();
            const price = row.sale_price.toLocaleLowerCase();

            return label.includes(needle) || price.includes(needle);
        });
    }, [optionLabelById, rows, search]);
    ...
    useEffect(() => {
        ...
        void fetchClientArticles(relationshipId)
            ...
            .catch(() => {
                if (!cancelled) {
                    setError(t('clients.articlesLoadFailed'));
                    setRows([]);
                    setArticleOptions([]);
                }
            })
        ...
        try {
            ...
            if (caught instanceof ClientArticlesApiError) {
                ...
                if (Object.keys(mapped).length === 0) {
                    setError(caught.message || t('clients.articlesSaveFailed'));
                }
            } else {
                setError(caught instanceof Error ? caught.message : t('clients.articlesSaveFailed'));
            }
        } finally {
            setSaving(false);
        }
    ...
    return (
        <BaseModal
            ...
            size="lg"
            title={t('clients.articlesModalTitle', { client: clientName })}
            description={t('clients.articlesModalDescription')}
            footer={
                <>
                ...
                <div className="space-y-3">
                    <div className="mb-4 space-y-1.5">
                        ...
                        <Input
                            ...
                            value={search}
                            disabled={saving}
                            placeholder={t('clients.articlesSearchPlaceholder')}
                    ...
                    {rows.length === 0 ? (
                        ...
                        <p className="rounded-lg border border-dashed border-line px-3 py-6 text-center text-sm text-ink-muted">
                            {t('clients.articlesNoSearchResults')}
                        </p>
                    ) : (
                        visibleRows.map((row) => {
