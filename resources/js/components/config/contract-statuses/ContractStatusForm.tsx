...
type ContractStatusFormProps = {
    ...
    errors: Partial<Record<keyof ContractStatusFormValues, string>>;
    processing: boolean;
    onChange: (key: keyof ContractStatusFormValues, value: string | boolean) => void;
...
export function ContractStatusForm({
    ...
    return (
        <FieldHelpScope table="contract_statuses">
            <form onSubmit={onSubmit} className="space-y-5 rounded-2xl border border-line bg-surface p-6 sm:p-8">
                ...
                <Field label={t('common.color')} htmlFor="color" error={errors.color}>
                    <div className="flex items-center gap-3">
                        ...
                        <Input
                            value={values.color}
                            placeholder={t('contractStatuses.colorPlaceholder')}
                ...
                <Field label={t('contractStatuses.lifecycle')} htmlFor="lifecycle" error={errors.lifecycle}>
                    <Input
                        id="lifecycle"
                        type="number"
                        min={0}
                    ...
                    />
                    <p className="text-xs text-ink-muted">{t('contractStatuses.lifecycleHint')}</p>
                </Field>

                <div className="space-y-1">
                    <p className="text-sm font-semibold text-ink">{t('contractStatuses.isOpen')}</p>
                    <p className="text-xs text-ink-muted">{t('contractStatuses.isOpenHint')}</p>
                    <Toggle
                        name="is_open"
                        checked={values.is_open}
                        onCheckedChange={(checked) => onChange('is_open', checked)}
                        checkedLabel={t('contractStatuses.isOpen')}
                        uncheckedLabel={t('contractStatuses.isOpenOff')}
                    />
