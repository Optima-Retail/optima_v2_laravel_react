...
type WorkOrderAttachmentsPanelProps = {
    ...
    onUpload: (id: number, file: File, options?: Record<string, unknown>) => void;
        ...
        <>
            <div className="space-y-4 rounded-2xl border border-line bg-surface p-6 sm:p-8">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <h2 className="text-base font-semibold text-ink">{t(`${labelsNamespace}.attachmentsTitle`)}</h2>
                        <p className="mt-1 text-sm text-ink-muted">{t(`${labelsNamespace}.attachmentsDescription`)}</p>
                    </div>
                ...
                {attachments.length === 0 ? (
                    <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-line px-4 py-10 text-center">
                        ...
                        <p className="text-sm text-ink-muted">{t(`${labelsNamespace}.attachmentsEmpty`)}</p>
                    </div>
                ) : (
