...
type ComplimentAttachmentsPanelProps = {
    complimentId: number;
...
export function ComplimentAttachmentsPanel({
    complimentId,
    attachments,
    can,
}: ComplimentAttachmentsPanelProps) {
    const { t, i18n } = useTranslation();
    const fileRef = useRef<HTMLInputElement>(null);
    const [uploading, setUploading] = useState(false);
    const [preview, setPreview] = useState<ComplimentAttachmentItem | null>(null);
    ...
    function upload(file: File | null | undefined) {
        ...
        setUploading(true);
        complimentsService.storeAttachment(complimentId, file, {
    ...
    async function remove(attachment: ComplimentAttachmentItem) {
        if (!can.delete_attachments) {
            return;
        }

        const confirmed = await confirmAction({
            title: t('common.deleteTitle', { resource: t('compliments.attachment') }),
        ...
        complimentsService.destroyAttachment(complimentId, attachment.id);
    }
        ...
        <>
            <div className="space-y-4 rounded-2xl border border-line bg-surface p-6 sm:p-8">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <h2 className="text-base font-semibold text-ink">{t('compliments.attachmentsTitle')}</h2>
                        <p className="mt-1 text-sm text-ink-muted">{t('compliments.attachmentsDescription')}</p>
                    </div>
                    ...
                    {can.upload_attachments ? (
                        <div>
                            ...
                            >
                                ...
                                {t('compliments.uploadAttachment')}
                            </Button>
                        </div>
                    ) : null}
                </div>
                ...
                {attachments.length === 0 ? (
                    <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-line px-4 py-10 text-center">
                        ...
                        <p className="text-sm text-ink-muted">{t('compliments.attachmentsEmpty')}</p>
                    </div>
                ) : (
                        ...
                        {attachments.map((attachment) => (
                            ...
                            >
                                ...
                                <div className="flex shrink-0 items-center gap-2">
                                    {attachment.is_image && can.download_attachments ? (
                                        <Button
                                            type="button"
                                            variant="secondary"
