...
namespace App\Domain\Config\ServiceTypes\Services;
...
final class ServiceTypeService
{
    ...
    public function paginateForWeb(array $filters = [], ?int $perPage = null): LengthAwarePaginator
    {
        return $this->paginate($filters, $perPage)
            ->through(fn (ServiceType $type): array => $this->toListItem($type));
    }
    ...
    {
        return DB::transaction(function () use ($type, $data): ServiceType {
            $type->update([
                'name' => $data['name'],
                'code' => $data['code'] ?: null,
                'color' => $data['color'] ?: null,
    ...
    public function delete(ServiceType $type): void
    {
        if ($type->trashed()) {
            return;
        }
     ...
     */
    public function toFormData(ServiceType $type): array
    {
        return [
            'id' => $type->id,
            'name' => $type->name,
     ...
     */
    public function toListItem(ServiceType $type): array
    {
        return [
            'id' => $type->id,
            'name' => $type->name,
